...
public class AvariceSettingsPage extends AbstractGDBServerSettingsPage implements
	...
	public void initializeFrom(ILaunchConfiguration configuration) {
		...
		try {
			...
			String jtaginterface = configuration
						.getAttribute(ATTR_GDBSERVER_AVARICE_INTERFACE,
					DEFAULT_GDBSERVER_AVARICE_INTERFACE);
			for (String[] interfaces : AVARICE_INTERFACES) {
				if (interfaces[1].equals(jtaginterface)) {
					fInterface.select(fInterface.indexOf(interfaces[0]));
					break;
				}
			}
	...
	public void performApply(ILaunchConfigurationWorkingCopy configuration) {
		...
		for (String[] interfaces : AVARICE_INTERFACES) {
			if (interfaces[0].equals(jtaginterface)) {
				configuration.setAttribute(ATTR_GDBSERVER_AVARICE_INTERFACE,
						interfaces[1]);
				break;
			}
